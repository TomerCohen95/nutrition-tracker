//
//  OptimizedDayView.swift
//  NutritionTracker
//
//  High-performance day view with targeted SwiftData queries
//
...
struct OptimizedDayView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $selectedItemToEdit, onDismiss: {
            print("✏️ OptimizedDayView dismissed edit sheet for date \(date)")
        }) { selection in
    ...
    private func presentEditSheet(for item: FoodItem) {
        print(
            "✏️ OptimizedDayView presenting edit sheet for \(item.name) [id=\(item.id.uuidString)] on date \(date)"
        )
